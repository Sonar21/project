...
useEffect(() => {
  ...
  const registerStudent = async () => {
    ...
    // You need to define or fetch these variables before using them
    // For demonstration, let's assume some default values
    const name = session.user?.name || "";
    const nameKana = "";
    const startMonth = new Date().toISOString().slice(0, 7);
    const entranceYear = new Date().getFullYear(); // Replace with actual logic
