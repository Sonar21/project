...
export function determineRoleByEmail(email) {
  ...
  // 1️⃣ School domain (osfl.ac.jp)
  if (domain.endsWith("osfl.ac.jp")) {
    if (/^[wk]\d+/i.test(local)) return "student"; // W24002 → student
    return "teacher"; // Tanaka → teacher
  }

  // 2️⃣ Gmail (development / test accounts)
  if (domain === "gmail.com") {
    // ---- Custom logic for your dev Gmail accounts ----
    const teacherEmails = [
      "thiriaung2003@gmail.com",  // ✅ mark specific emails as teacher
      "roseicooper96@gmail.com",
      "teacher@gmail.com"
    ];

    if (teacherEmails.includes(email.toLowerCase())) return "teacher";

    // fallback — anything with 'teacher' in local part is teacher
